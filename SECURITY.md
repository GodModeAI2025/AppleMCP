...
- **Bridge to app.** The Unix socket, and three checks in front of it (`Sources/M3MCPCore/LocalHTTPServer.swift`, `SocketAuthorizer.swift`, `PeerIdentity.swift`).
  ...
  0. *Availability.* A connection that has not yet delivered a complete request is read through a dispatch source and holds no thread; it gets `requestTimeout` seconds to finish, then `408` and a close. At most 128 connections wait and at most 32 requests are served at once. When all 128 are taken, the arriving connection is not the one refused: the connection that has waited longest without sending a byte gives up its slot, and only when every slot holds a request that is genuinely being read does the new one get `503`. Working out who the peer is happens when a complete request arrives, not at `accept`, because the accept loop is the only thing draining the listen backlog and a signature check in front of it is a `connect` the kernel refuses for somebody else. This is listed first because it sits before the two checks below: the token is a header, so nothing can be authorized until the request has been read, and until then a connection has to be cheap.
...
**The token is a bearer secret wherever it is configured as one.** `M3MCP_TOKEN` in `claude_desktop_config.json` is readable by every process of the user, so copying that file is enough. The keychain is the fallback when no environment variable is set, and it is a narrow one: the item's ACL names the app that created it, and the bridge does not ask to be added — it refuses a read it is not already entitled to, because a panel it cannot show is a hang. So the fallback carries a bridge whose hash is already on the ACL and no other, and in practice the token lives in the client's config file. See the keychain paragraph below.

**A local process can still make the endpoint work for its living.** A connection that has not delivered a request costs a descriptor and a deadline rather than a thread, and when the slots are full it is the oldest silent one that goes, so silence buys an attacker nothing it can hold. Measured against the shape it used to fail on: 900 connections that connect and say nothing, `/health` and an authenticated tool call both answered in 0.12 seconds, and under four threads reconnecting in a loop for six seconds 98 to 100 percent of tool calls came back `200`. What is left is not nothing. Every one of those connections still costs an `accept`, a descriptor and a dispatch source until its deadline, the endpoint runs slower while it is under load, and a process that can open sockets faster than the kernel's listen backlog of 128 drains will still have some `connect` calls refused, its own and other people's alike. Nothing here distinguishes a client that holds the token from one that does not, because the token is a header and the header has not arrived yet. The bound is on what the endpoint spends, not on who may make it spend.

**The pin identifies the binary, not the caller.** A copied token plus `M3MCP.app/Contents/MacOS/M3MCPBridge` is a working client for anything that can run both, because that bridge is exactly the binary the pin trusts. `SocketAuthenticationTests.testThePinRefusesAHandwrittenClientAndPassesTheBundledBridge` measures it: pinned to the bridge's code directory hash, a handwritten socket client with the right token gets `403`, and the same token handed to the bundled bridge by the same process comes back with a tool result. So the pin is worth what it is worth and no more: it forces an attacker through the shipped binary instead of a client of their own, which without the token gets them nothing and with the token gets them everything. Read any sentence promising that a copied token is refused as false; there is no such promise here.
...
**The keychain fallback does not carry a client that was started headless.** Measured with an item written by one binary and read by `M3MCPBridge`: `SecItemCopyMatching` had not returned after 25 seconds and the bridge had printed nothing, because the ACL panel was waiting somewhere no MCP client can show it. `kSecUseAuthenticationUI` does not govern that panel — `…UIFail` and `…UISkip` both still hung — so `CapabilityToken.read` turns interaction off with `SecKeychainSetUserInteractionAllowed` for the client path only, and the same read now comes back in 15 milliseconds as `errSecAuthFailed` with a message naming `M3MCP_TOKEN`. What that leaves, and it is a real cost: the bridge now never asks, so the keychain fallback reaches only an item whose ACL already names this bridge binary. Every other client holds its token in a file on disk. Giving the app's item an ACL that names the sibling bridge would close that — `TrustedClient` already finds the binary — but it means `SecAccessCreate`, deprecated, against a hash that changes with every rebuild. Open. `SocketAuthenticationTests.testTheBridgeAnswersWhenItMayNotReadTheKeychainItem` keeps the hang from coming back.
