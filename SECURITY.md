...
**The token is a bearer secret wherever it is configured as one.** `M3MCP_TOKEN` in `claude_desktop_config.json` is readable by every process of the user, so copying that file is enough. The keychain path is better — another binary asking for the item produces a prompt naming the asker — and it is what the bridge uses when no environment variable is set.
