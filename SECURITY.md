...
- **Bridge to app.** The Unix socket, and three checks in front of it (`Sources/M3MCPCore/LocalHTTPServer.swift`, `SocketAuthorizer.swift`, `PeerIdentity.swift`).
  ...
  0. *Availability.* A connection that has not yet delivered a complete request is read through a dispatch source and holds no thread; it gets `requestTimeout` seconds to finish, then `408` and a close. At most 128 connections wait and at most 32 requests are served at once, anything beyond that is `503`. This is listed first because it sits before the two checks below: the token is a header, so nothing can be authorized until the request has been read, and until then a connection has to be cheap.
...
**A local process can still fill the connection slots.** A connection that has not delivered a request costs a descriptor and a deadline rather than a thread, which is what makes `/health` and authenticated tool calls survive a flood of silent connections — measured at 120 of them, answered in 0.04 seconds where it used to run into an eight second timeout. It is a bound, not immunity: 128 connections held open occupy `maximumOpenConnections`, and the 129th, whoever it belongs to, is refused with `503` until a deadline expires ten seconds later. A process that reconnects in a loop keeps it there. Nothing here distinguishes a client that holds the token from one that does not, because the token is a header and the header has not arrived yet.
