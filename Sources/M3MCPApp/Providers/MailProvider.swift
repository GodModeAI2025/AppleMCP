...
final class MailProvider {
    ...
    private func locateEnvelopeIndex() throws -> URL {
        ...
        do {
            ...
            throw MailStoreFailure("Cannot inspect the local Mail store at \(mailRoot.path). Select an accessible data folder in Permissions, or leave this optional source disabled. Detail: \(error.localizedDescription)")
        }
        try checkCancellation(.requestBoundary)
    ...
    private func withDatabase<T>(at url: URL, body: (OpaquePointer) throws -> T) throws -> T {
        ...
        guard status == SQLITE_OK, let database else {
            ...
            throw MailStoreFailure("Cannot read the local Mail index. Select an accessible data folder in Permissions, or leave this optional source disabled. Detail: \(message)")
        }
