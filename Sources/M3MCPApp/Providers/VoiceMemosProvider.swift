...
final class VoiceMemosProvider {
        ...
        do {
            ...
            guard let data = try? recording.read(maximumBytes: maxBytes) else {
                return ToolResponse(
                    ok: false,
                    source: sourceName,
                    message: "Cannot read \(fileURL.path). Select an accessible data folder in Permissions, or leave this optional source disabled."
                )
            }
    ...
    private func locateStore() throws -> RecordingStore {
        ...
        throw VoiceMemoStoreFailure("The Voice Memos store was not found below \(primary.path). Select an accessible data folder in Permissions, or leave this optional source disabled.")
        #endif
    }
    ...
    func withDatabase<T>(
        ...
        guard status == SQLITE_OK, let database else {
            ...
            throw VoiceMemoStoreFailure("Cannot read the Voice Memos store. Select an accessible data folder in Permissions, or leave this optional source disabled. Detail: \(message)")
        }
        ...
        do {
            ...
            throw VoiceMemoStoreFailure(
                "Cannot prepare a safe temporary copy of the Voice Memos store at \(url.path). "
                    + "Select an accessible data folder in Permissions, or leave this optional source disabled. "
