...
public struct SocketAuthorizer: Sendable {
    ...
    /// `/health` is the one path that answers without a token: it is the documented probe, and the
    /// server keeps its reply free of anything that is not already public (see `LocalHTTPServer`,
    /// which strips the activity log from it).
