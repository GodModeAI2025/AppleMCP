...
///
/// There is no pairing dialog and no Developer ID to pin against, so the answer has to come from
/// something the installation already knows for certain: the `M3MCPBridge` that sits next to the
/// app's own executable. That holds in both places the app ever runs from — `.build/<config>/` for a
/// source build and `M3MCP.app/Contents/MacOS/` for the packaged bundle — and it is exactly the
...
public enum TrustedClient {
    ...
    public static func resolve(appExecutableURL: URL?) -> Resolution {
        ...
        guard FileManager.default.isExecutableFile(atPath: bridge.path) else {
            return Resolution(
                hashes: [],
                note: "no \(bridgeExecutableName) next to \(appExecutableURL.lastPathComponent), so the client binary is not pinned"
            )
        }
